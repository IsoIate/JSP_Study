...
+ 자신보다 변하기 쉬운 것에 의존하던 것을 추상화된 인터페이스나 상위 클래스를 두어 변하기 쉬운 것의 변화에 영향을 받지 않도록 하는 것

+ DIP 원칙 적용 전 (자주 변경되는 구체 클래스에 의존)
  - 자동차 -> 스노우타이어 
  - 자동차는 한번 사면 몇 년은 타야하는데 스노우타이어는 계절이 바끼면 일반 타이어로 교체해야 함
  - 스노우타이어를 일반타이어로 교체할 때 자동차는 그 영향에 노출되어 있음

+ DIP 원칙 적용 후
  - 자동차 -> (인터페이스) 타이어 - 스노우타이어, 일반타이어 등에서 구현
  - 자동차가 구체적 타이어들이 아닌 추상화된 타이어 인터페이스에 의존함으로써 타이어 교체 시 영향을 받지 않음
  - 이전에는 스노우타이어가 그 무엇에도 의존하지 않았지만 DIP 원칙 적용 후 타이어 인터페이스에 의존함으로써 의존 방향이 역전됨
